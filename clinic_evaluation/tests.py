...
class ClinicEvaluationTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        clinic_evaluation=ClinicEvaluation.objects.create(patient=patient1, registered_by=user, nails="strong", skin= "normal", bladder_habits="normal", bowel_habits="normal", additional_information="test", date_of_consultation=datetime.date(2021, 11, 14), updated=datetime.date(2021, 11 , 14))
    ...
    def test_clinic_evaluation(self):
        ...
        self.assertEqual(clinic_evaluation.date_of_consultation, datetime.date(2021, 11, 14), "The date of consultation registered correctly")
        self.assertEqual(clinic_evaluation.updated, datetime.date(2021, 11, 14), "This field registered correctly")
        self.assertEqual(clinic_evaluation.additional_information, "test", "This fiels was registered correctly")

    def test_lab_exam(self):
        lab_exam=LabExam.objects.create( name_exam ="exame de sangue", date_of_exam=datetime.date(2021, 11, 14), upload="test", exam_information= "exemplo glicemia=180", clinic_evaluation=ClinicEvaluation.objects.first() )
        lab_exam.save()
        lab_exam = LabExam.objects.first()
        self.assertEqual(lab_exam.name_exam, "exame de sangue", "The name was registered correctly")
        self.assertEqual(lab_exam.date_of_exam, datetime.date(2021, 11, 14), "The field date of exam registered correctly")
